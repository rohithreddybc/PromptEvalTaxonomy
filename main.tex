...
model.
Mirzadeh et al.~\cite{mirzadeh2024gsmsymbolic} report up to 65\%
performance drops on a single landmark reasoning benchmark (GSM8K)
when only numerical values in the questions are altered, reinforcing
that single-phrasing results from one model can shift dramatically
under semantics-preserving perturbations. Mizrahi et
al.~\cite{mizrahi2024state} and Salinas \&
...
of several percentage points under paraphrase. Vaugrante et
al.~\cite{vaugrante2024replication} further report that the
reasoning improvements claimed for chain-of-thought,
EmotionPrompting, and ExpertPrompting do not reach statistical
significance once replicated across six models on six reasoning
benchmarks, suggesting that FM3's prevalence is not merely a
reporting style but a replicability concern. Cross-model findings
