...
In four years, large language models (LLMs) have moved from academic
novelties to foundational components of production software, driving
substantial research on prompt engineering (PE): crafting natural-language
inputs to elicit reliable outputs without modifying
weights~\cite{liu2021pretrain, schulhoff2024prompt}. Chain-of-thought,
retrieval-augmented generation, automated prompt optimization, and
dozens of related techniques have been proposed and deployed across
tasks from arithmetic reasoning to clinical decision
support~\cite{wei2022chain, lewis2020retrieval, sahoo2024systematic}.

A concrete scenario illustrates why this matters. A chain-of-thought
variant for medical differential diagnosis is evaluated by Study~A on
MedQA with exact-match accuracy (D1=Benchmark, D3=Task-specific,
D4=Automated) and by Study~B with GPT-4 as a clinical scoring judge
on a different question set (D1=LLM-as-Judge, D3=LLM-judge,
D4=Automated). Both report improvements, but a clinician choosing
between them cannot combine the results: A's exact-match accuracy
does not capture the reasoning or safety properties clinical
deployment requires; B's judge score is uninterpretable without
calibration against clinical ground truth. Neither study fails
internally; both are limited by an evaluation ecosystem without a
shared vocabulary for what each measured, the tradeoffs each
accepted, and the conditions under which results
transfer~\cite{chang2023survey, guo2023evaluating}. Proposing such a
vocabulary as a seven-dimensional taxonomy grounded in measurement
theory, and testing whether it organizes the literature usefully, is
the contribution of this paper.

\subsection*{Contribution}

This paper reorganizes prompt engineering evaluation practice into a
measurement-theoretic frame, together with the empirical work needed
to test whether the reorganization is well-posed. The contribution
is four independent, individually verifiable items:
...
\textit{What we integrate, and what is new at the integration.}
Three streams of prior survey work each contribute a piece that the
present taxonomy combines into a single coding scheme.
PE-techniques surveys~\cite{schulhoff2024prompt, sahoo2024systematic,
vatsal2024survey, liu2026taxonomy} catalog methods through
dimensional category labels; we apply that practice to the
\emph{evaluation step} rather than to prompt construction.
LLM-evaluation surveys~\cite{chang2023survey, ni2025benchmarks,
mohammadi2025agents, gu2025llmjudge} treat benchmarks, agents, and
LLM-judges along multiple axes; we rebuild it around PE-specific
dimensions and promote robustness (D6) and reproducibility (D7) to
first-class axes (those surveys cover both in passing). The
construct-validity literature on LLM benchmarks~\cite{bean2025measuring,
alaa2025medical, ye2025psychometrics} supplies the
measurement-theoretic framing; we apply it to PE evaluation
configurations and operationalize the tension through corpus-grounded
archetypes and failure modes. The integration produces a structure
none of the source works supplies: a seven-dimensional codebook that
classifies the evaluation choices PE studies make (not the prompts),
validated through convergent clustering criteria, and connected to
three named failure modes whose corpus prevalence (FM1: 7.2\%, FM2:
83.3\% of LLM-judge papers, FM3: 29.6\%) is reproducible from the
released coding. The new insight is the joint application of these
three lenses to PE evaluation: under that joint application, the
literature partitions into recurring archetypes whose downstream
reporting properties (D6 and D7) the archetype label statistically
predicts (Cram\'er's $V = 0.21$ and $0.33$;
Section~\ref{subsubsec:predictive}).

\textit{How this paper differs from each closely related survey.}
The contribution is sharpest when stated as a per-survey contrast.
Our work differs from \textbf{Schulhoff et al.}~\cite{schulhoff2024prompt}
(The Prompt Report; PRISMA on 1{,}565 papers) in that they catalog
58 \emph{prompting techniques} via a flat 6-group taxonomy, whereas
we taxonomize the \emph{evaluation configurations} under which
techniques are assessed (D1--D7), independent of which technique a
study evaluates. We differ from \textbf{Sahoo et
al.}~\cite{sahoo2024systematic} in that they organize methods by
application domain, whereas we organize evaluations by
measurement-theoretic dimensions. We differ from \textbf{Vatsal \&
Dubey}~\cite{vatsal2024survey} in that they organize methods by NLP
task type; we orthogonalize task scope (D2) as a moderator of the
metric--validity tradeoff rather than as the primary axis. We differ
from \textbf{Liu et al.}~\cite{liu2026taxonomy} (closest title-overlap,
FCS~2026) in that they taxonomize \emph{prompt designs} along four
aspects (profile/instruction, knowledge, reasoning, output-bias
reliability), whereas we taxonomize the \emph{evaluation
configurations} along seven dimensions and reserve ``reliability'' for
its measurement-theory sense. We differ from \textbf{Bean et
al.}~\cite{bean2025measuring} (445 LLM benchmarks reviewed for
construct validity) in that they apply construct-validity review to
benchmarks broadly, whereas we apply it to PE evaluation
configurations specifically and add archetype clustering, failure-mode
catalogs, and predictive validation against D6/D7 reporting. We
differ from \textbf{Ye et al.}~\cite{ye2025psychometrics} in that
they survey LLM psychometrics across capabilities; we apply
psychometric concepts to PE evaluation-design choices. We differ
from \textbf{Chang et al.}~\cite{chang2023survey} in that their
what/where/how decomposition is descriptive; ours is tension-based
(validity/reliability and reference/judgment) and corpus-grounded
through 152 coded papers. We differ from \textbf{Gu et
al.}~\cite{gu2025llmjudge} in that they survey LLM-as-Judge bias as
a self-contained topic; we treat LLM-judge as one of seven
co-evolving dimensions and add D6 (robustness) and D7
(reproducibility) as first-class axes that judge surveys treat in
passing. We differ from \textbf{Ni et al.}~\cite{ni2025benchmarks}
in that they sort 283 benchmarks into a three-tier taxonomy; we
classify the evaluation methodology applied to PE work, of which
benchmark choice is one axis (D1). We differ from \textbf{Mohammadi
et al.}~\cite{mohammadi2025agents} and \textbf{Yehudai et
al.}~\cite{yehudai2025agents} in that they propose two- and
five-dimensional agent-evaluation taxonomies for agentic systems;
ours is seven-dimensional and PE-specific. We differ from
\textbf{Liu, Xiaoxia et al.}~\cite{liu2024promptingframeworks} (in
this venue) in that they survey prompting frameworks in the sense of
toolkits and lifecycle structures (Data, Base, Execute, Service
Levels); we taxonomize the evaluation step. The common thread of
our improvement across these comparisons is the explicit application
of a measurement-theoretic frame to PE evaluation configurations,
producing corpus-derived archetypes and failure modes that none of
the above supplies.

The taxonomy is not a new evaluation method but a classification
scheme with empirical findings on how those methods co-occur. We
claim contribution on the proposed organization and empirical
analyses, not field-level priority on measurement-theoretic LLM
evaluation in general (see Section~\ref{subsec:using}).
...
Sections~\ref{subsec:tension_scalability}--\ref{subsec:tension_acrosstime}
...
Three streams bear on this survey: PE methods, LLM evaluation
frameworks, and NLP taxonomies. We characterize each stream's
...
shown to shift model outputs~\cite{brown2020gpt3}.
Few-shot~\cite{brown2020gpt3} and
chain-of-thought~\cite{wei2022chain} established the foundational
paradigms; the technique catalog now includes zero-shot
CoT~\cite{kojima2022large},
self-consistency~\cite{wang2023selfconsistency},
tree-~\cite{yao2023tree} and graph-of-thought~\cite{graphofthoughts2023,
demystifying2024}, least-to-most~\cite{leasttomost2022},
plan-and-solve~\cite{plansolve2023}, faithful CoT~\cite{faithfulcot2023},
many-shot ICL~\cite{manyshot2024},
AutoPrompt~\cite{shin2020autoprompt}, FLAN~\cite{wei2022flan},
prompt-pattern catalogs~\cite{white2023prompt}, manual-prompt
studies~\cite{manualpromptnotdead2025}, RAG~\cite{lewis2020retrieval},
and automated optimization
(GEPA~\cite{gepa2025}, PromptWizard~\cite{promptwizard2024}). Recent automated-prompt-optimization
...
separate topics with no systematic bridge. On the PE methods side,
Schulhoff et al.~\cite{schulhoff2024prompt} catalog 58 prompting
techniques (meta-analysis, no evaluation framework); Sahoo et
al.~\cite{sahoo2024systematic} and Vatsal \&
...
knowledge, reasoning/planning, output-bias reliability) that mirrors
our title structure but classifies prompts not their evaluations;
Wen et al.~\cite{pettax2025} taxonomize template structures;
Fagbohun et al.~\cite{fagbohun2024categorization} provide a
seven-category practitioner guide; Sasaki et al.~\cite{petaxse2025}
cover SE-specific patterns; Tian et al.~\cite{petaxdefects2025}
catalog prompt defects in code; He et al.~\cite{contextengineer2025}
situate PE within ``context engineering''; Liu et
al.~\cite{liu2024promptingframeworks}, in this venue, survey
\emph{prompting frameworks} as toolkits and lifecycle structures
rather than evaluation frameworks; Plaat et
al.~\cite{plaat2024multistep}, also in this venue, survey
multi-step reasoning along generation/evaluation/control axes; and
Dong et al.~\cite{dong2022survey} provide the foundational
...
152-paper PRISMA corpus. Read together, these three streams supply pieces that our taxonomy
combines into a single coding scheme. The technique-survey stream
contributes the practice of dimensional labelling; the LLM-evaluation
stream contributes the multi-axis treatment of evaluation; the
construct-validity stream contributes the measurement-theoretic
framing. None of the streams in isolation addresses the question we
foreground: for a researcher evaluating a specific prompting
technique, which evaluation framework should they use, and why?
The integration produces that classification: dimensions chosen for
PE work specifically (D1--D7), validity/reliability framing applied
to evaluation configurations rather than to benchmarks, and
empirical archetypes plus failure modes derived from the integrated
view. The remainder of this section makes the integration
boundaries explicit.

\subsection{Differentiation from Prior Surveys}
\label{subsec:differentiation}

Several recent surveys overlap in topic with this paper; the
question is what the integration produces beyond reorganizing known
evaluation practices. Table~\ref{tab:differentiation} positions this
work against eleven closely related surveys on three axes: (1)
...
Section~\ref{sec:taxonomy}. Each prior row supplies a piece used in
our integration: \checkmark{} marks coverage that this work absorbs
directly; $\circ$ marks coverage we extend by re-anchoring it to
PE evaluation specifically; \textendash{} marks an axis our taxonomy
adds at the integration step.
...
\textit{Takeaway:} This paper is the only one combining a PE-technique catalog, an evaluation-method catalog, and an explicit measurement-theoretic frame applied to PE. The closest title-overlap is Liu et al.~\cite{liu2026taxonomy} (techniques, not evaluations); the closest framing-overlap is Bean et al.~\cite{bean2025measuring} and Ye et al.~\cite{ye2025psychometrics} (LLM benchmarks and capabilities, not PE configurations).}
\label{tab:differentiation}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{2.5cm} p{1.4cm} p{3.1cm} p{0.5cm} p{0.5cm} p{0.5cm} p{0.5cm} p{0.5cm} p{0.5cm} p{0.5cm}}
\toprule
...
Three differences are central. First, no compared survey organizes
PE \emph{evaluation} around a measurement-theoretic frame. Bean et
al.~\cite{bean2025measuring} and Ye et al.~\cite{ye2025psychometrics}
apply construct-validity and psychometric framing respectively, but
to LLM benchmarks broadly and LLM capabilities; Chang et al.'s
what/where/how is descriptive, not tension-based. Second, no compared
survey treats robustness (D6) and reproducibility / contamination
resistance (D7) as first-class dimensions of evaluation design (Ni
et al.\ on contamination and Gu et al.\ on judge bias treat them as
sub-findings). Third, prior PE-focused surveys with similar titles
(Schulhoff, Sahoo, Vatsal \& Dubey, Liu et al.~2026) catalog
\emph{methods} not \emph{evaluation frameworks}; the title-overlap
with Liu et al.~2026 is purely lexical (their four aspects classify
prompt designs; our seven classify the evaluation configurations
under which prompts are tested). Budagam et
al.~\cite{budagam2024hierarchical} use ``evaluation framework'' for
a single hierarchical prompting scheme, not a taxonomy of existing
...
LLMs are transformer-based~\cite{vaswani2017attention} networks
trained on web-scale text; at scale they handle broad task ranges
without task-specific retraining~\cite{devlin2019bert, brown2020gpt3,
bommasani2021foundation}. Practitioners interact through prompts, and
surface phrasing produces nontrivial output
differences~\cite{sclar2023formatting, zhou2022ape}. Prompt
engineering (designing inputs to obtain reliable outputs without
modifying weights) has developed its own benchmarks, optimization
algorithms, and taxonomies~\cite{sahoo2024systematic, schulhoff2024prompt}.
...
Four metric families recur. \textbf{Reference-based metrics}
(BLEU~\cite{papineni2002bleu}, ROUGE~\cite{lin2004rouge}) score by
surface overlap against a gold reference; fast and reproducible but
weakly aligned with human judgment on open-ended
tasks~\cite{liu2023geval, repairing2022}. \textbf{Model-based
metrics} (BERTScore~\cite{zhang2020bertscore}) use learned semantic
representations, handling paraphrase more gracefully but still
requiring a reference~\cite{chang2023survey}. \textbf{LLM-as-Judge
metrics} (G-Eval~\cite{liu2023geval}) delegate scoring to a second
LLM, eliminating the gold-reference requirement at the cost of
judge biases (positional, length,
self-enhancement)~\cite{zheng2023judging, gu2025llmjudge}.
\textbf{Task-specific metrics} (pass@$k$ for code, exact match for
factual retrieval, RAGAS~\cite{rageval2024} for RAG pipelines) are
purpose-built and rarely transfer.
Section~\ref{sec:taxonomy} organizes these along D3.

\subsection{Validity and Reliability in Evaluation}
\label{subsec:validityreliability}

We use \emph{reliability} (consistency under repetition) and
\emph{construct validity} (the degree to which a score is evidence
about the property it intends to
capture)~\cite{cronbach1955construct, messick1995validity} in their
measurement-theory sense. The two typically pull against each other:
exact-match scoring against a fixed key is highly reliable but
narrow on ``answer quality''; expert-panel scoring of open-ended
outputs pursues higher construct validity at the cost of speed,
replicability, and inter-rater agreement~\cite{chang2023survey}. The
translation to ML/NLP evaluation~\cite{jacobs2021measurement,
subramonian2023takeoneforteam, liao2023designing} is the spine of
the dimensional decomposition in Section~\ref{sec:taxonomy}.
...
choices cluster as they do. The two tensions we propose as an
\emph{interpretive frame} are not original to this paper; they
integrate (i) the construct-validity vs.\ measurement-reliability
distinction long established in psychometrics
\cite{cronbach1955construct, messick1995validity} and recently
re-applied to ML measurement~\cite{jacobs2021measurement,
bean2025measuring, ye2025psychometrics}, with (ii) the
reference- vs.\ judgment-anchored distinction implicit in the
LLM-judge survey literature~\cite{gu2025llmjudge, li2025generationjudgment}.
Our integration is to apply this two-axis frame to the dimensional
choices PE evaluation studies make, formalized in
Section~\ref{subsec:validityreliability} and consistent with the corpus
...
plausible; we choose these because the seven dimensions project onto
them in identifiable ways and the prior streams cited above each
ground one axis. Whether this frame has explanatory force
...
\textit{What the frame organizes.} The two tensions render three
later corpus patterns interpretable: D1/D3 are entangled (Cram\'er's
$V = 0.702$) because the judgment locus largely fixes the metric
family; benchmark-automation is modal on closed-form tasks because
automation pays near-zero construct cost when the reference
approximates the construct; LLM-judge adoption fragments because
moving to a proprietary judge without calibration breaks
across-paper comparability. Whether the frame has explanatory force
beyond organization is examined in
...
scope, metric type, automation level, evaluation scope). The
extension to seven is not an additive innovation but an integration:
the prompt-sensitivity literature~\cite{sclar2023formatting,
promptsensitivity2025, butterfly2024, mizrahi2024state,
zhuo2024prosa, errica2024sensitivity, vaugrante2024replication} and
the benchmark-contamination literature~\cite{ni2025benchmarks,
feng2025sage, mcintosh2024inadequacies, balloccu2024leakcheat}
already treat robustness and reproducibility as first-class
evaluation concerns; we promote them from corpus findings inside
other surveys to top-level dimensions (D6, D7) of a PE-evaluation
taxonomy. The five-to-seven extension also reframes
\textit{D4 (Automation Level)} as a sub-axis of reliability rather
than a peer dimension, making both underlying tensions traceable
through every coding decision.
...
\textit{Takeaway:} The corpus clusters in the high-reliability, low-construct-validity quadrant (Benchmark-Automation archetype), suggesting benchmark-automation emphasizes measurement convenience over construct coverage.}
...
The taxonomy is useful only when applied. Subsection~\ref{subsec:classify}
classifies two corpus papers; Subsection~\ref{subsec:design} walks a
new study, surfacing implicit decisions;
Subsection~\ref{subsec:quickref} distills the guidance into a
reference table.
...
\textbf{Wei et al., Chain-of-Thought~\cite{wei2022chain}.}
D1=Benchmark (GSM8K, SVAMP); D2=Reasoning (arithmetic, symbolic,
commonsense); D3=Task-specific (exact-match accuracy);
D4=Automated (script-based against references); D5=Cross-model
(PaLM 540B, GPT-3, LaMDA); D6=Single-prompt (one template per
benchmark); D7=Artifact-released (prompts and exemplars disclosed;
not contamination-checked at publication).

\textbf{Zheng et al., MT-Bench~\cite{zheng2023judging}.}
D1=LLM-as-Judge (GPT-4 scoring on 1-10 scale); D2=Multi-task
(eight categories: writing, roleplay, math, coding, etc.);
D3=LLM-judge (GPT-4 judgments; calibration reported against human
pairwise preferences on held-out subset); D4=Automated (MT-Bench
component; Chatbot Arena human preferences are adjunct);
D5=Cross-model (28 open and proprietary models); D6=Multi-prompt
aggregate (positional and verbosity bias probes; no per-prompt
sensitivity bounds); D7=Artifact-released, Judge-version-pinned
(GPT-4 snapshot identified; not contamination-checked).
...
\textit{D1 - Evaluation method.} Curated gold-answer sets for clinical
QA are scarce, domain-constrained, and legally sensitive; human or
hybrid evaluation is more defensible than D1\,=\,Benchmark, which
should document why the chosen benchmark covers clinical accuracy.

\textit{D2 - Task scope.} D2\,=\,Clinical makes the
sparsely-represented positioning explicit and signals that general-NLP
evaluation instruments may not transfer without adaptation.

\textit{D3 - Metric.} Reference-based metrics (BLEU, ROUGE) are
structurally inadequate without a gold reference; model-based factual
precision (e.g., FActScore~\cite{factscore2023}) is defensible with a
well-defined knowledge source. D3\,=\,LLM-judge is an alternative but
requires calibration against clinician judgments before scores carry
clinical interpretability.

\textit{D4 - Automation level.} Full automation reduces cost but
risks systematic errors clinicians would catch. Human-in-loop is more
appropriate for a first evaluation where no calibrated automated
metric exists.

\textit{D5 - Evaluation scope.} D5\,=\,Single-model gives limited
generalizability evidence; D5\,=\,Cross-model is the stronger basis
for deployment claims across systems.

\textit{D6 - Robustness reporting.} Clinical RAG is exposed to prompt
sensitivity because retrieved-context phrasing varies across patient
records. D6\,=\,Sensitivity-bounded (variance across paraphrased
clinical queries on a held-out set) is defensible;
D6\,=\,Single-prompt should be flagged as a generalizability risk.

\textit{D7 - Reproducibility commitments.}
\textit{Judge-version-pinned} (for LLM components) and
\textit{contamination-checked} (test set not in training corpus)
are stronger choices than D7\,=\,Not-reported, especially since
medical benchmarks are smaller and easier to contaminate.

Making these decisions explicit before evaluation reduces post-hoc
rationalization and makes the study directly comparable to prior
work.
...
dimension. Each row names the tradeoff that dimension forces. Full
preregistration checklist in Electronic Supplement (Appendix~A).
\textit{Takeaway:} A ``no'' answer is not a disqualifier but
benefits from justification; without it, the dimension defaults to
implicit benchmark choice rather than deliberate design.}
\label{tab:quickref}
\centering
\footnotesize
...
\textit{Takeaway:} No single tool covers all seven dimensions; D6 (robustness) and D7 (reproducibility) are most commonly unaddressed, requiring researchers to combine frameworks or accept systematic gaps.}
\label{tab:tools}
\centering
\footnotesize
...
These tools partition the design space along the two tensions:
benchmark frameworks (Archetype~A) maximize reliability but accept
narrow construct validity; LLM-as-Judge frameworks (Archetype~B)
broaden coverage but introduce judge-side variance that robustness
tools (PromptBench, PromptSET, JudgeSense) begin to quantify. No
single tool covers all seven dimensions simultaneously, mirroring
the archetype-tradeoff finding in
Section~\ref{subsec:archetypes}.

% ---------------------------------------------------------------
\section{Methodology: Systematic Literature Review}
\label{sec:methodology}
The 152-paper corpus follows PRISMA 2020~\cite{page2021prisma}: one
primary search round (Sections~\ref{subsec:search}--\ref{subsec:screening},
Figure~\ref{fig:prisma}) plus four targeted expansion rounds
(Table~\ref{tab:expansion_rounds}). All five rounds apply PRISMA
conventions (stated databases, documented strategy, explicit
inclusion criteria (Section~\ref{subsec:inclusion}), dual-reviewer
screening at $\kappa$ = 0.81, per-round identified/screened/
excluded/included counts, dual coding at $\kappa$ = 0.76--0.92). The
primary round used broad PE-evaluation queries; expansion rounds
used narrower topical refinements documented as ``additional records
from supplementary searches'' within the PRISMA flow. These
coverage-repair rounds are not independent prevalence sweeps; the
directional bias is named in
Section~\ref{subsubsec:lim_expansion}.

\subsection{Search Strategy}
\label{subsec:search}

We searched six databases: arXiv (cs.CL, cs.AI), ACM Digital
Library, IEEE Xplore, Semantic Scholar, Scopus, and Web of
Science~\cite{kitchenham2007slr}. The primary search ran in
2025-Q1--Q2; four expansion rounds followed in 2025-Q3 to 2026-Q2
(Table~\ref{tab:expansion_rounds}) with no lower year cutoff
(nearly all retained papers postdate 2020).

The base query (adapted per database, full strings in the
repository at \url{https://github.com/rohithreddybc/PromptEvalTaxonomy}):

\noindent{\small\ttfamily (``prompt engineering'' OR ``prompting technique'' OR ``prompt optimization'' OR ``chain-of-thought'' OR ``few-shot prompting'' OR ``zero-shot prompting'' OR ``retrieval-augmented generation'') AND (``evaluation'' OR ``benchmark'' OR ``assessment'' OR ``metric'' OR ``framework'' OR ``human evaluation'') AND (``large language model'' OR ``LLM'' OR ``GPT'' OR ``language model'').}
...
\textbf{Re-admittance criterion (rounds 2--3).} The initial round
excluded papers whose \emph{topic} was prompt injection,
jailbreaking, or narrow clinical/legal application; rounds 2--3
re-admitted them when their \emph{evaluation methodology} is what
the taxonomy classifies (e.g., prompt-injection benchmarks and
MIRAGE/MedRAG~\cite{xiong2024medrag} contribute evaluation
infrastructure). Re-admitted papers are flagged in Appendix~C.

The use-as-tool exclusion is practically consequential: many papers
use prompting incidentally without evaluating the prompting
approach; these were excluded as they do not contribute to
understanding evaluation frameworks.

\textbf{On mixing paper types.} The corpus spans four overlapping
categories (PE techniques, benchmarks/datasets, LLM-as-Judge and
metric-quality, PE/LLM-evaluation surveys) unified by participation
in PE evaluation infrastructure, not paper genre. All distributions
in Section~\ref{sec:comparison} are aggregate from
\texttt{corpus\_d1d7.csv} and independently verifiable; subgroup
contrasts (e.g., LLM-as-Judge for D7) are noted inline. Recent CSUR
systematic reviews use the same mixed-category convention
(e.g.,~\cite{edemacu2024privacysurvey, chang2023survey}).

\textbf{On including arXiv preprints.} Preprints are retained only
when they meet at least one quality condition: 10+ Google Scholar
citations at screening, publication at a recognized NLP/AI venue, or
citation-trace recovery from an already-included peer-reviewed paper.
The venue-criterion step excluded 72 records. Two 2026
preprints~\cite{bellibatlu2026judgesense, benchmark2sq2026} are
flagged in Section~\ref{subsubsec:lim_recency} as preliminary; no
headline claim rests solely on a 2026 preprint.

\textbf{On corpus size.} The 152-paper corpus is comparable to
CSUR-tier systematic surveys (142 in~\cite{edemacu2024privacysurvey},
${\sim}160$ in~\cite{sahoo2024systematic}, ${\sim}220$
in~\cite{chang2023survey}) and narrower than omnibus surveys because
dimensional coding (D1--D7) requires per-paper full-text review and
dual coding. Sensitivity analysis (Section~\ref{subsec:sensitivity})
confirms claim stability; per-dimension $\kappa$
(Table~\ref{tab:kappa}) documents coding reliability.
...
\textbf{Corpus publication-status breakdown.} All 152 papers meet at
least one quality condition (peer review, 10+ Google Scholar
citations at screening, or recognized NLP/AI venue). A precise
peer-reviewed vs.\ arXiv split is not reported because many corpus
papers were posted to arXiv first and later accepted at venues
(NeurIPS, ACL, EMNLP, ICLR, TACL) while retaining the arXiv
identifier in their \texttt{journal} field; the per-paper record is
...
in Table~\ref{tab:expansion_rounds}). Two reviewers independently screened all titles and abstracts
against the criteria. Disagreements were resolved through discussion
(percent agreement 94.1\%, $\kappa$ = 0.81 over a 200-record
dual-screened sample); unresolved cases were
...
\textit{Takeaway:} Expansion rounds 1--4 contributed 73 of 152 papers (48.0\%); a single broad primary search substantially underestimates coverage for fast-moving topics (LLM-judge quality, contamination, agentic evaluation).}
...
\midrule
1 & Few papers explicitly studying judge reliability despite 30 corpus papers (in the final 152) using LLM-judge methods & Exploratory & 20 & Over-represents LLM-judge quality and bias literature \\
...
\textit{Takeaway:} Of 3,297 identified records, 130 (3.9\%) met inclusion via database search; 22 added via citation tracing in Round~0 yielded the 152-paper corpus. The high full-text exclusion rate (732) reflects papers that use prompting as a tool without evaluating it.}
...
Table~\ref{tab:kappa} reports per-dimension agreement. Dimensions
split into two regimes: D1--D5 reach $\kappa$ = 0.84--0.92 (``almost
perfect'', Landis--Koch threshold $>0.81$); D6--D7 reach
$\kappa$ = 0.76--0.78 (``substantial''). The lower D6/D7 values
reflect boundary ambiguity rather than factual disagreement (e.g.,
papers acknowledging sensitivity without quantifying it sit between
D6=Not-reported and D6=Single-prompt). The heterogeneous profile is
itself the finding: some properties are codable from descriptions,
others require interpretive judgment that a reporting standard could
reduce.
...
Mapping the 152-paper corpus against the integrated taxonomy reveals
patterns that no single prior survey makes visible: the joint
distribution over D1--D7 surfaces recurring \textit{evaluation
archetypes}, dimension-pair dependencies, and three failure modes
where a paper's configuration does not fully support its
contribution claim. Each pattern is a product of the integration in
Section~\ref{sec:taxonomy}; we report them here and ground the
...
Combining technique, benchmark, LLM-judge, and survey papers aids the
taxonomy's expressive test (dimensional categories must cover all
four types if the framework is to be general) but trades that breadth
for straightforward prevalence interpretation. Three implications.
First, aggregate percentages describe the \emph{distribution of
evaluation configurations represented under the inclusion criteria of
Section~\ref{subsec:inclusion}}, not field-level prevalence; excluding
surveys or weighting by citation count would yield different
percentages. Second, archetype shares are sensitive to the PE-core /
infrastructure split: Archetype B concentrates in the
LLM-judge-frameworks population (Section~\ref{subsec:archetypes}), so
its share partly reflects how many such frameworks meet inclusion
...
per-category breakdown (Table~\ref{tab:comparison}). The expansion
rounds make the corpus partially purposive rather than purely
systematic; selection-bias implications appear in
Section~\ref{subsubsec:lim_expansion}.
...
D7 configuration.
\textit{Takeaway:} Under strict mutual-exclusion, Benchmark-Automation accounts for 66.4\% (95\% CI [58.6\%, 73.7\%]) and Judge-Mediated for 17.1\% [11.2\%, 23.7\%]; the choice between them is the central design distinction in this corpus.}
...
\textit{Takeaway:} All categories default to automated, cross-model evaluation; the primary differentiators are metric type (D3) and artifact release (D7), where LLM-judge papers are the only group with majority artifact release.}
...
Benchmark-based evaluation accounts for 110 papers (72.4\%), the
default across every subgroup including foundation-model technical
reports for GPT-4~\cite{achiam2023gpt4},
LLaMA~\cite{touvron2023llama, touvron2023llama2}, and
GPT-4o~\cite{hurst2024gpt4o}. Benchmarks are reproducible and
cross-lab comparable but cannot capture output quality on open-ended
tasks without a single correct answer.

LLM-as-Judge appears in 30 papers (19.7\%), concentrated in
open-ended generation and instruction-following
...
judge panels and meta-evaluation~\cite{verga2024replacing,
chern2024scaleeval, multiagentjudge2025}, two
surveys~\cite{gu2025llmjudge, li2024llmsjudges}, bias
...
Human evaluation appears in 14 papers (9.2\%), ablation in 2 (1.3\%);
no primary user studies. Four papers~\cite{llmvshuman2024,
alternativeannotator2025, humansorllm2024, canlllmsreplace2023} are
coded under both LLM-as-Judge and Human, so subcategory counts sum
to 156 (headline frequencies use the 152-paper denominator). The
overall distribution favours scalable, reliable configurations over
the construct-validity guarantees human evaluation provides
...
exact match, F1, accuracy), mirroring benchmark dominance in D1.
LLM-judge metrics appear in 31 (20.4\%); G-Eval~\cite{liu2023geval}
marked the shift to LLM-based scoring on open-ended outputs. Hybrid
metrics in 16 (10.5\%); model-based (BERTScore family) in 2 (1.3\%),
with~\cite{li2024leveraging} documenting the gradual shift to
LLM-judge. Reference-based metrics (BLEU, ROUGE) appear as primary
measures in zero corpus papers, a reversal of historical dominance
following the lexical-overlap vs.\ human-judgment gap documented
by~\cite{repairing2022}.

\subsection{Distribution across Automation Level (Dimension 4)}
\label{subsec:d4dist}

Fully automated evaluation is modal at 124 papers (81.6\%); hybrid
19 (12.5\%), manual 5 (3.3\%), human-in-loop 4 (2.6\%). The
concentration reflects cost and throughput pressures and carries a
D7 implication: pipelines depending on proprietary judges are
stable within-run but not across-time, since the judge can change
between API versions~\cite{feng2025sage}.
...
45 papers (29.6\%) carry the FM3 fingerprint (D5=Cross-model with
D6 $\in$ \{Single-prompt, Not-reported\}), including
chain-of-thought~\cite{wei2022chain} and zero-shot
CoT~\cite{kojima2022large}. Cross-model claims under single-prompt
support narrower conclusions than their phrasing suggests.

\subsection{Co-occurrence Patterns Across Dimensions}
\label{subsec:cooccurrence}

The seven dimensions are conceptually independent but far from
statistically independent. Table~\ref{tab:cooccurrence} reports
pairwise co-occurrence for subcategory pairs the
validity-reliability frame anticipates as linked.
...
\textit{Takeaway:} Benchmark bundles metric and automation choices (D1+D3 = D1+D4 = 66.4\%); LLM-as-Judge similarly bundles with LLM-judge metric (19.7\%), confirming the judge-mediated archetype.}
...
D1/D3 and D5/D6 entanglement are not coding artifacts but
properties the dimensional structure makes visible: a benchmark
paper typically adopts the benchmark's metric and forgoes
paraphrase-variance reporting. The validity-reliability frame
organizes these clusters around coherent design-space regions with
the same tradeoffs.
...
three-archetype labelling on the 141 archetype-fitting papers is
...
\emph{Check 2: agreement with unsupervised k-means.} k-means for
$k=2,\ldots,6$ on the one-hot vectors (k-means++ init, 10 seeds per
$k$, lowest-SSE) with no knowledge of the rule-based labels yields
silhouette $0.335$ at $k=3$ ($0.284$ at $k=2$, $0.349$ at $k=4$) and
ARI $=0.51$ between the unsupervised $k=3$ partition and the
rule-based labels. $k=3$ is silhouette-competitive (parsimony
favours it over $k=4$); the partitions agree moderately rather than
recovering a different structure.

\emph{Caveat.} Both checks use a one-hot encoding of the same D1--D7
codes used to derive the rule-based labels, so they validate cluster
separability and recovery under the coding, not natural-kind
existence. ARI $=0.51$ is moderate, not high: the conclusion is
that the three-archetype structure is robust to rule-based vs.\
unsupervised labelling under this coding.
...
Nori et al.~\cite{nori2023gpt4medical} carries the same FM1
fingerprint on clinical reasoning, and Hou et
al.~\cite{advancedllmsse2024} on code exhibits a closely related
construct-mismatch pattern (outside the FM1 D2 subset but identical
in structure): the benchmark metric supports answer-key agreement
claims, but the deployment-readiness claim about clinical or
code-quality behaviour requires Archetype-B or Archetype-C evidence
(or a calibrated reference-free metric) that the cited papers do
not provide. Liu et
...
prevalence column.
\textit{Takeaway:} Each FM has a one-step reporting-change mitigation (no new evaluation method needed). 45.4\% of the corpus exhibits at least one FM; modes are largely non-overlapping (no paper has all three), so targeted reporting changes per FM can substantially reduce the configuration--claim gap.}
\label{tab:fmsummary}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\midrule
FM2 & Unpinned Judge & D1$\supseteq$LLM-as-Judge; D7 lacks JP & 25 / 30 LLM-judge = 83.3\% (16.4\% corpus) & Record judge model + snapshot date; pin checkpoint when possible & Panickssery~\cite{judgingjudges2024} \\
\midrule
...
within-run vs.\ across-time refinement (Definition~3). Each tension
below integrates a prior literature stream with the PE corpus:
construct validity vs.\ reliability draws on
psychometrics~\cite{cronbach1955construct, messick1995validity,
jacobs2021measurement, ye2025psychometrics} and applies it to PE
configurations; reference- vs.\ judgment-anchored draws on the
LLM-as-Judge survey literature~\cite{gu2025llmjudge,
li2025generationjudgment} and applies it to whole-study reporting
choices rather than to individual judge instances; within-run vs.\
across-time draws on the contamination and judge-drift
literature~\cite{balloccu2024leakcheat, feng2025sage,
bellibatlu2026judgesense} and operationalizes its concerns through
D7. The subsections name each tension, point to corpus evidence,
and identify what the integration suggests is missing.
...
\label{subsec:tension_acrosstime}

A pipeline can be reliable within-run yet fragile across-time
(infrastructure drifts between API revisions; test data leaks into
pretraining). D7 captures this asymmetry: artifact release is
common, but contamination-checking and judge-version pinning are
not (Section~\ref{subsec:d7dist}). The tension is structural: static
benchmarks grow more contaminated with continued training, and
LLM-judge pipelines cannot pin proprietary judges. Recent work on
dynamic contamination-resistant benchmarks~\cite{wildbench2024} and
judge-sensitivity quantification~\cite{bellibatlu2026judgesense,
feng2025sage} addresses parts; broader uptake is the open question.

\subsection{Open Problems by Tension}
\label{subsec:open_problems}

Each tension implies an open problem. The scalability vs.\ validity
tension implies \textit{calibrated reference-free metrics} for tasks
with no gold answer, validated against an expert standard with
calibration evidence reported alongside scores;
FActScore~\cite{factscore2023} and the alternative annotator
test~\cite{alternativeannotator2025} provide building blocks, but a
systematic domain-specific PE evaluation literature does not yet
exist (with the corpus caveats from Section~\ref{subsec:d2dist}).
The reference vs.\ judgment tension implies a \textit{shared LLM-judge
calibration standard}: minimum disclosure (judge model, version,
prompt template, calibration outcome). The within-run vs.\
across-time tension implies \textit{infrastructure-stable pipelines}:
dynamic contamination-resistant benchmarks, locally-pinned judge
models where licensing permits, or at minimum explicit snapshot-date
reporting for proprietary components.
...
Papers from 2025--2026 without citations or a recognized venue fall
outside retention criteria; the corpus may underrepresent recent
methodological developments in LLM-as-Judge and dynamic-benchmark
subfields. The expansion rounds extended search cutoffs to mitigate
this, but the citation-threshold recency bias remains. Two 2026
arXiv preprints~\cite{bellibatlu2026judgesense, benchmark2sq2026}
are treated as preliminary and never used as sole support for a
claim.

\subsubsection{Coding-Judgment Ambiguity at Boundary Cases}
\label{subsubsec:lim_coding}
D4, D6, and D7 required interpretation when papers did not
explicitly describe these properties. The inter-rater statistics
(Section~\ref{subsec:coding_reliability}) quantify the resulting
uncertainty per dimension; the sensitivity analysis
(Section~\ref{subsec:sensitivity}) shows how much propagates to
headline claims.
...
The four expansion rounds (Table~\ref{tab:expansion_rounds}) used
narrower topical queries on underrepresented areas (LLM-judge
quality, agentic evaluation, foundation-model reports, long-context
tasks). This introduces directional selection bias: targeted topics
are over-represented relative to their true field prevalence, and
topics absent from all five rounds remain underrepresented.
Distributional findings should not be extrapolated to topic areas
absent from the five rounds; cross-round prevalence comparisons
...
One direction deserves more space. \emph{Reasoning-first models}
(o1, o3, DeepSeek-R1) challenge three assumptions: D1 cannot
attribute performance to prompt design versus autonomous internal
CoT; D3 faces an open measurement problem in trace faithfulness;
D6 has a new variance source from internal CoT sampling invisible
to single-prompt reporting. Whether D1/D3/D6 require new
subcategories or updated operational definitions is a priority for
...
The taxonomy is not a complete solution. It does not settle which
evaluation method is optimal for a given task, nor supply domain
expertise for specialized protocols. It names tradeoffs that go
unnamed, locates them in the literature, and offers a vocabulary in
which methodological standardization is at least describable.
Adoption is a community decision; disclosing a D1--D7 configuration
...
decide, not for this paper to prescribe. What this paper offers, at
the close, is the integration: the D1--D7 codebook, FM1--FM3
catalog, and three archetype labels are intended to be used together
as a single tool. Subsequent work that adopts any one of these can
reference the others from the same source; subsequent work that
re-codes a different corpus can compare its archetype shares against
ours through the released coding scheme. The integration is the
artifact; the catalog of pieces is incidental.
...
reproducible via the Python scikit-learn script
\texttt{cluster\_validation.py} in the repository; alternative-clustering ARIs
...
probability) reproduce within $\pm 0.05$ under the documented random
seed (RNG\_SEED~=~42) and one-hot encoding; small deviations across
scikit-learn versions are expected and documented in the repository
\texttt{README}.
