...
concerns that the surveyed work has not yet managed with a shared calibration
...
metric choice yields very different validity properties depending on
whether the task admits a single gold answer (arithmetic, exact
retrieval) or only a distribution of acceptable answers (open-ended
generation, clinical decision support). The eight primary categories
are: \emph{Reasoning} (arithmetic, symbolic, commonsense, multi-step
logic --- the most heavily studied category since chain-of-thought
prompting~\cite{wei2022chain} originated here); \emph{Natural
language generation} (summarization, dialogue, translation, open-ended
writing; admits no single correct answer); \emph{Code generation}
(pass@$k$ against test suites~\cite{scoT2023, promptalchemy2025});
\emph{Question answering} (closed- and open-book retrieval, scored
with exact match or F1); \emph{Clinical and domain-specific} (medical
QA, clinical notes, legal reasoning; higher accuracy requirements and
domain-expert evaluation~\cite{clinicalnlp2024, medicalcot2025});
\emph{Multilingual} (cross-language performance, often surfacing gaps
monolingual evaluations
conceal~\cite{vatsal2024survey}); \emph{Agentic and tool-use}
(trajectory-oriented success-rate over multi-step
workflows~\cite{yao2024taubench}); and \emph{Multimodal} (vision-,
audio-, and video-grounded prompted
evaluation~\cite{fu2023mme}). Papers spanning multiple categories
without specializing are coded \emph{Multi-task} as a cross-category
label. Agentic and Multimodal entered the taxonomy during expansion
rounds 2--3 (Table~\ref{tab:expansion_rounds}) and remain smaller
populations.
...
under semantically equivalent prompt rephrasings? Robustness is a
property of the construct, not the measurement instrument, and
therefore projects onto the validity axis: a technique that performs
well under one phrasing but degrades sharply under paraphrase is not,
in a methodologically meaningful sense, the technique the paper
claims to evaluate. Existing literature provides empirical
motivation: Sclar et al.~\cite{sclar2023formatting} showed formatting
changes shift accuracy enough to reverse leaderboard rankings;
Salinas and Morstatter~\cite{butterfly2024} document similar
paraphrase variance; PromptSET~\cite{promptsensitivity2025} provides
an explicit sensitivity benchmark; Bellibaltu et
al.~\cite{bellibatlu2026judgesense} (2026 arXiv preprint;
preliminary) extend the analysis to LLM-judge pipelines, reporting an
8.5\% coherence flip rate on GPT-4o under paraphrase and over 60\% on
smaller judges. The four subcategories index the depth of
sensitivity reporting: \emph{Not reported}, \emph{Single-prompt}
(one phrasing, no variance), \emph{Multi-prompt aggregate}
(averaged across paraphrases without per-prompt breakdown), and
\emph{Sensitivity-bounded} (variance or stability bounds reported
explicitly).
...
pretraining~\cite{ni2025benchmarks, raji2021ai}). The four
subcategories are coded as non-mutually-exclusive binary properties:
\emph{Artifact-incomplete} (no released code/prompts/seeds),
\emph{Artifact-released} (code, prompts, seeds available),
\emph{Contamination-checked} (test items verified absent from
training data via held-out construction~\cite{wildbench2024} or
contamination probes), and \emph{Judge-version-pinned} (API version
or local checkpoint of any LLM-judge component recorded and
recoverable).
...
regimes: objectively verifiable dimensions (D1, D2, D3, D5) reach
$\kappa$ = 0.88--0.92 (``almost perfect'' on the Landis--Koch scale);
interpretively coded dimensions (D4, D6, D7) reach $\kappa$ =
0.76--0.84 (``substantial''). The lower $\kappa$ on D6 and D7
...
threshold and should be read with that caveat. All other claims move
by less than 2~pp.
...
Benchmark-based evaluation accounts for 106 papers (69.7\%),
...
Human evaluation appears in 16 papers (10.5\%), ablation studies in
2 (1.3\%); no corpus paper uses a user study as primary method. Four
papers in the LLM-judge-and-human cluster (Ye et
al.~\cite{llmvshuman2024}, Calderon et
al.~\cite{alternativeannotator2025},
Ye et al.~\cite{humansorllm2024}, Chiang et
al.~\cite{canlllmsreplace2023}) are coded under both D1=LLM-judge and
D1=Human, so D1 subcategory counts sum to 156 rather than 152; the
headline frequencies use the 152-paper denominator. The
human-evaluation cluster splits into primary-signal
studies~\cite{jang2023consistency, bubeck2023sparks},
safety-audit work~\cite{chang2025redteamingmed, steyvers2024calibgap},
and human-vs-LLM cognitive
comparisons~\cite{strachan2024theoryofmind}; LLMBar~\cite{zeng2024llmbar}
provides complementary instruction-following measurement. Read
against the overall distribution, the corpus favours configurations
that maximize reliability and scale over those that prioritize the
construct-validity guarantees human evaluation provides
...
task-specific subgroup at 18 papers (11.8\%), reflecting
...
in 10 papers (6.6\%), code in 12 (7.9\%), QA in 8 (5.3\%).

Clinical and multilingual coverage are reported separately to avoid
conflating task domain with language scope. By \emph{primary
target}, clinical is 1 paper (0.9\%; Sivarajkumar et
al.~\cite{sivarajkumar2024empirical}) and multilingual is 2 (1.8\%;
\cite{vatsal2025multilingual, son2024mmeval}); by \emph{any-occurrence}
the shares are 3.9\% and 2.6\% respectively
(Table~\ref{tab:sensitivity}), adding MIRAGE/MedRAG, Nori et al.\
USMLE, Chang et al.\ medical red-teaming, Kim et al.\ medical
hallucination, MGSM, and MM-Eval. Agentic evaluation
\cite{liu2023agentbench, ma2024agentboard, liu2024toolace}, safety
and prompt-injection benchmarks \cite{liu2023houyi,
maloyan2025judgevuln}, legal evaluation \cite{guha2023legalbench,
fei2023lawbench}, medical evaluation \cite{nori2023gpt4medical,
chang2025redteamingmed, kim2025medicalhallu}, instruction-following
\cite{zhou2023instruction}, multimodal \cite{fu2023mme,
liu2025multimodalhallu}, foundational reasoning benchmarks
\cite{cobbe2021gsm8k, mirzadeh2024gsmsymbolic, shi2022mgsm,
lin2021truthfulqa}, code generation \cite{jimenez2023swebench,
liu2023evalplus, jain2024livecodebench, roziere2023codellama,
promptingsecurecode2024, impactpromptprogramming2024}, long-context
\cite{hsieh2024ruler, bai2023longbench, yen2024helmet}, and reward
modelling \cite{lambert2024rewardbench, malik2025rewardbench2} round
out the corpus.

Under our inclusion criteria --- which excluded papers that use
prompting as a tool without analyzing it as the studied object ---
the corpus remains strongly concentrated on multi-task benchmarks
with limited clinical or multilingual coverage. Domain-specific
prompting work exists extensively in the broader
literature~\cite{clinicalnlp2024, medicalcot2025, vatsal2024survey};
the relative scarcity here is a property of this corpus under these
inclusion criteria, not a field-level estimate.

\subsection{Distribution across Metric Type (Dimension 3)}
\label{subsec:d3dist}

Task-specific metrics dominate at 101 papers (66.4\%) --- the largest
...
dimensions are mutually reinforcing. LLM-judge metrics appear in 33
papers (21.7\%), essentially the D1=LLM-as-judge set; G-Eval
\cite{liu2023geval} marked the shift toward LLM-based scoring on
open-ended outputs. Hybrid metrics appear in 5 papers (3.3\%);
...
al.~\cite{repairing2022}. The remaining 11 papers (7.2\%) are surveys
and pattern catalogs where D3 is coded Not-reported.

\subsection{Distribution across Automation Level (Dimension 4)}
\label{subsec:d4dist}

Fully automated evaluation is the modal configuration: 123 papers
(80.9\%) run assessments without any human involvement at any stage.
Hybrid (15 papers, 9.9\%), human-in-loop (8, 5.3\%), and fully manual
(6, 3.9\%) make up the remainder. The concentration at the automated
...
Cross-model evaluation is the most frequent scope at 88 papers
(57.9\%): more than half the corpus tests findings across multiple
model families. Cross-domain appears in 30 papers (19.7\%),
concentrated in the survey subgroup (which spans task types by
design). Single-model evaluation appears in 18 (11.8\%), cross-dataset
in 8 (5.3\%), and cross-lingual in 2 (\cite{vatsal2025multilingual,
son2024mmeval}); percentages sum to less than 100\% because some
multi-scope studies are counted under their primary scope only. The
18 single-model papers warrant attention: a technique evaluated on
one model at one point in time on one benchmark produces a claim
about performance in that configuration alone, and
Mizrahi et al.~\cite{mizrahi2024state} have documented that such
evaluations routinely overstate effects relative to multi-prompt
...
(55.9\%) and the contamination-checked share (8.6\%). A paper that
...
LLM-judge papers.} Of the 41 D7=Not-reported papers, 28 (68.3\%) are
D1=Benchmark; only 4 are D1=LLM-as-judge / LLM-judge. The LLM-judge
cluster has the higher artifact-release norm
(Table~\ref{tab:d7dist}: 65.6\% vs.\ 55.9\% overall) and is also
where Judge-version-pinning concentrates (15.6\% within the cluster
...
\textit{Sensitivity reporting concentrates where the technique is
the studied object, not where the judge is.} 54 of 152 papers
(35.5\%) report multi-prompt aggregate or sensitivity-bounded
robustness, but 36 of those (66.7\%) are D1=Benchmark technique
papers. Only 7 of the 32 LLM-judge papers (21.9\%) report
sensitivity at all on the judge --- and 12 of the 32 (37.5\%) use a
single-prompt judge evaluation, including AlpacaEval~2.0
\cite{dubois2024alpacaeval}, G-Eval \cite{liu2023geval}, and FActScore
\cite{factscore2023}. The judge's own sensitivity to paraphrase is
under-tested compared with the techniques it is asked to score.
...
infrastructure) is time-stable. The 63\% figure is not a function
of paper category; it is distributed across all four categories
of Table~\ref{tab:comparison}.
...
PE require reference-free metrics calibrated against expert
annotations, task-specific rubrics vetted by domain professionals,
and multilingual judge models that are not English-dominant.
Contamination-resistant dynamic benchmarks~\cite{wildbench2024}
provide one architectural template. Clinical evaluation
\cite{sivarajkumar2024empirical, nori2023gpt4medical,
chang2025redteamingmed, kim2025medicalhallu}, legal evaluation
\cite{guha2023legalbench, fei2023lawbench}, and RAG-based
domain-specific evaluation \cite{xiong2024medrag, saadfalcon2023ares,
chen2023rgb, yu2024ragsurvey, zhao2024aigcrag} occupy this direction
in the corpus.

\subsubsection{A Shared LLM-Judge Calibration Standard}
\label{subsubsec:fd_calibration}
Studies reporting LLM-as-judge scores would benefit from a
calibration experiment against a held-out human-annotated sample on
the specific task, with disclosure of judge model identity, prompt
template, and calibration outcome. The alternative annotator
test~\cite{alternativeannotator2025} offers a principled statistical
foundation; PoLL~\cite{verga2024replacing} and
ScaleEval~\cite{chern2024scaleeval} provide architectural patterns.
The broader calibration literature
\cite{geng2023calibration, xiong2023uncertainty, steyvers2024calibgap}
documents that users systematically overestimate LLM confidence even
when models are well-calibrated, which strengthens the case for
required calibration-metadata disclosure.

\subsubsection{Cross-Model, Cross-Dataset, and Cross-Paraphrase Scope as a Minimum Bar}
\label{subsubsec:fd_scope}
Single-model, single-prompt results would be more usefully read as
exploratory than as generalizable. Demonstrating consistent
performance across at least two model families, two benchmarks
within the target task type, and two prompt paraphrases would close
much of the gap between phrasing and supportable claim
\cite{mizrahi2024state, sclar2023formatting, promptsensitivity2025,
butterfly2024}.

\subsubsection{Robustness as a First-Class Evaluation Dimension}
\label{subsubsec:fd_robustness}
The sensitivity literature has documented paraphrase-induced variance
large enough to reverse study conclusions; the JSS work of Bellibatlu
et al.~\cite{bellibatlu2026judgesense} extends the concern to the
judge itself with a 500-pair benchmark across four evaluation tasks
on 13 judge models. Their finding that scale fails to predict judge
consistency is the kind of result the corpus currently lacks
conventions to report. Reporting sensitivity bounds alongside point
estimates (the D6=Single-prompt vs.\ D6=Sensitivity-bounded
distinction) is a practical change; PromptEval~\cite{polo2024promptseval}
shows how to make such reporting budget-efficient on the task-solver
side.

\subsubsection{Multimodal and Agentic PE Evaluation Frameworks}
\label{subsubsec:fd_multimodal}
\label{subsec:multimodal}
Multimodal prompting (image+text, audio+text) and multi-step agentic
pipelines do not map cleanly onto the three archetypes. D2 has been
extended to include Agentic and Multimodal subcategories
(§\ref{subsec:d2}), but the D1/D3/D4 configurations dominating
Archetype~A were designed for single-turn text-only settings.
MME~\cite{fu2023mme} and $\tau$-bench~\cite{yao2024taubench} are
representative corpus instances; as agentic systems become primary
PE deployment contexts, companion D1, D3, and D6 conventions for
multi-step task completion, tool use, and inter-agent consistency
remain a near-term direction.

\subsubsection{Standardized Evaluation Reporting Templates}
\label{subsubsec:fd_reporting}
A community-endorsed minimum-bar reporting template --- analogous to
CONSORT~\cite{raji2021ai} or the NLG-eval norms of Gehrmann et
al.~\cite{repairing2022} --- would make existing results comparable
without methodological consensus. The Electronic Supplement
Appendix~A checklist is one such template; a venue-endorsed version
adopted at major NLP/AI conferences alongside hardware, model-version,
and seed reporting would be the natural next step.

\subsubsection{Safety, Bias, and Adversarial Robustness Evaluation}
\label{subsubsec:fd_safety}
PE evaluation frameworks have largely treated safety and bias as
adjacent to task completion rather than as first-class evaluation
dimensions. The jailbreak literature
\cite{wei2023jailbroken, yu2023gptfuzzer, bhardwaj2023redeval,
huang2023catastrophic, sharma2025constitutional, yi2024jailbreaksurvey}
shows that deployed LLMs remain vulnerable to adversarial prompting
after safety training; the bias and fairness literature
\cite{gallegos2023bias, esiobu2023robbie} makes a parallel argument
for demographic robustness. Evaluation reports of new prompting
techniques would benefit from a structured adversarial-robustness
probe and a demographic-bias measurement alongside accuracy claims.
...
For D3, the faithfulness of the exposed reasoning trace to the
actual computation is an open measurement problem; task-specific
metrics on the final answer may conceal trace-level errors. For D6,
within-prompt stochasticity from internal CoT sampling adds a
variance source invisible to single-prompt reporting: identical
prompts can elicit different reasoning paths and different answers
across invocations even at temperature zero. Whether D1/D3/D6
require new subcategories or can accommodate these with updated
operational definitions is a priority for the next survey cycle.
...
silhouette 0.398 against a random-baseline 0.04 (§\ref{subsubsec:clustervalidation}),
indicating the structure is real rather than imposed; Cram\'er's $V$
