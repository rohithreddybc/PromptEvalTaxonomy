...
findings about how those methods co-occur in the literature. We are
not aware of a prior survey that organizes prompt engineering
evaluation through a measurement-theoretic lens, though the survey
landscape is fast-moving; the contribution we claim is the proposed
organization and the empirical analyses that follow from it
(Section~\ref{subsec:using}), not field-level priority.
...
ranged from 0.27 to 0.34. GMM-BIC was minimized at $k=3$
(BIC$_{k=3}=-10653$ vs.\ $-9898$ at $k=2$ and $-10019$ at $k=4$),
...
\subsubsection{Predictive validation: does the archetype carry
information beyond its construction?}
...
from D1 and D4 alone) statistically predict D6 (robustness) and D7
(reproducibility) properties, which were not used in archetype
construction. A chi-square test of independence between archetype
membership and D6 distribution yields $\chi^2 = 13.53$ ($p = 0.035$,
Cram\'er's $V = 0.219$); the same test against D7 distribution yields
$\chi^2 = 36.08$ ($p < 0.001$, Cram\'er's $V = 0.358$). The strongest
predictive signal is on D7: judge-version pinning is reported by 0\%
of Archetype A papers (0/101) but 16.7\% of Archetype B papers
(5/30), and full non-reporting of D7 properties holds for 80\% of
Archetype C papers (8/10) versus 25.7\% of Archetype A (26/101).
The taxonomy therefore carries information about downstream reporting
practices beyond what its construction inputs encode, which is
consistent with (though does not by itself establish) the claim that
the archetype structure reflects coherent evaluation strategies
rather than coding artefacts.
...
metric on a non-exact-match construct.} Dimensional fingerprint: D1=Benchmark,
D3$\in$\{Task-specific, Reference-based, Model-based\}, applied to a
D2 task category whose construct is not well-approximated by a single
closed-form reference (D2$\in$\{Generation, Clinical, Agentic,
Multilingual\}). \emph{Corpus prevalence:} 11 of 152 papers (7.2\%)
under the strict fingerprint, derived directly from the released
\texttt{corpus\_d1d7.csv}. Named exemplars: Sivarajkumar et
al.~\cite{sivarajkumar2024empirical} (empirical clinical prompting
evaluation; D2=Clinical, D3=Task-specific, D6=Multi-prompt-aggregate,
D7=Not-reported), Nori et al.~\cite{nori2023gpt4medical} (GPT-4
medical capabilities; D2=Clinical, D3=Task-specific, D5=Single-model,
D7=Not-reported), and the cross-model SE evaluations of Hou et
al.~\cite{advancedllmsse2024} (D2=Code, D3=Task-specific,
D5=Cross-model, D6=Not-reported, D7=Not-reported; coded under the
broader fingerprint that includes Code with task-specific metrics). The configurations support claims about answer-key
...
without judge-version pinning.} Dimensional fingerprint:
D1$\supseteq$LLM-as-Judge, D7 lacks the Judge-version-pinned property.
\emph{Corpus prevalence:} 25 of 30 LLM-judge papers (83.3\% of the
LLM-judge subgroup, 16.4\% of the full corpus) carry this fingerprint.
Named exemplars:
...
single-prompt cross-model claim.} Dimensional fingerprint: D5=Cross-model
with D6$\in$\{Single-prompt, Not-reported\}.
\emph{Corpus prevalence:} 45 of 152 papers (29.6\%) carry this
fingerprint (42 with D6=Single-prompt, 3 with D6=Not-reported),
making FM3 the most prevalent of the three failure modes and
endemic to the modal Archetype A configuration. Named exemplars:
chain-of-thought prompting~\cite{wei2022chain} (D5=Cross-model,
D6=Single-prompt) and zero-shot CoT~\cite{kojima2022large} (same
fingerprint), both landmark techniques whose original evaluations
report cross-model generalization on a single prompt phrasing per
model.
...
overlap is modest (FM1$\cap$FM3 = 2 papers; FM2$\cap$FM3 = 9 papers;
no paper exhibits all three). These are corpus-derived counts, not
...
For citing work that finds the D1--D7 labels useful, three
applications follow naturally:
(1)~\emph{Methodology disclosure}: embed the seven-tuple in a single
methodology line (e.g., ``D1=Benchmark, D2=Reasoning,
D3=Task-specific, D4=Automated, D5=Cross-model, D6=Multi-prompt
aggregate, D7=Artifact-released'');
(2)~\emph{Comparative claims}: use the machine-readable corpus dataset
(review-time snapshot with submission; see Data Availability) as a
comparison reference rather than re-coding papers;
(3)~\emph{Evaluation design}: use the Electronic Supplement
(Appendix~A) checklist as a preregistration template. None of these
are prescriptive uses; whether they prove sufficient is for the
community to assess.
