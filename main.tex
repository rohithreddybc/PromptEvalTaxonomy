...
build a template-structure taxonomy; Sasaki et al.~\cite{petaxse2025} cover
...
\textit{Takeaway:} Among the compared surveys, this paper is the only one to combine a prompting-technique catalog, an evaluation-method catalog, and an explicit measurement-theoretic frame, making its coverage structurally distinct from the nine prior surveys included in the comparison. The closest title-overlap is with Liu et al.~\cite{liu2026taxonomy} (``A comprehensive taxonomy of prompt engineering techniques for large language models'', FCS 2026), which taxonomizes \emph{techniques} along four orthogonal aspects (profile/instruction, knowledge, reasoning/planning, and reliability defined as output-bias reduction); the present survey instead taxonomizes \emph{how techniques are evaluated} and reserves ``reliability'' for its measurement-theory meaning.}
