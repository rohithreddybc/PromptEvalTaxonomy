...
\textit{Takeaway:} Among the compared surveys, this paper is the only one to combine a prompting-technique catalog, an evaluation-method catalog, and an explicit measurement-theoretic frame, making its coverage structurally distinct from the six prior surveys included in the comparison.}
...
as a cross-category label; Agentic and Multimodal are smaller
populations added in expansion rounds 2--3.
...
canlllmsreplace2023} are coded under both D1=LLM-judge and D1=Human,
...
Clinical (primary target: 1 paper, 0.9\%) and multilingual (primary:
2 papers, 1.8\%) are small populations by primary coding; by
any-occurrence they rise to 3.9\% and 2.6\% respectively
(Table~\ref{tab:sensitivity}). The full per-paper breakdown is in
...
(57.9\%). Cross-domain appears in 30 (19.7\%), concentrated in the
survey subgroup. Single-model evaluation appears in 18 (11.8\%),
cross-dataset in 8 (5.3\%), cross-lingual in
2~\cite{vatsal2025multilingual, son2024mmeval}. Single-model findings
support claims about that configuration alone; single-prompt,
single-model evaluations systematically overstate effects relative to
multi-prompt baselines on the same model~\cite{mizrahi2024state}.
...
D1=LLM-as-Judge, D7=Artifact-released only), Ye et al.~\cite{llmvshuman2024}
