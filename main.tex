...
choices researchers make; it should help interpret why those choices cluster
the way they do. We frame the configuration space of prompt engineering
evaluation as structured by two largely orthogonal tensions, both
formalized in Section~\ref{subsec:validityreliability}, each visible in
the distribution of practice across the corpus
(Section~\ref{sec:comparison}):
(1)~\textit{construct validity} vs.\ \textit{measurement reliability}
(Definitions~2--3: does the score capture the intended property, and does
it do so consistently?), and
(2)~\textit{reference-anchored} vs.\ \textit{judgment-anchored} evaluation
(Definitions~4--5: is scoring determined by a stored gold answer, or
delegated to an evaluator whose output is not pre-determined?).
The seven dimensions in Sections~\ref{subsec:d1}--\ref{subsec:d7}
project onto these axes in specific, identifiable ways, and the
configurations that papers actually adopt cluster in regions of the space
the validity/reliability frame anticipates.
Figure~\ref{fig:taxonomy} positions the dimensions on the two-axis space;
Section~\ref{sec:comparison} then locates 152 papers within it.
