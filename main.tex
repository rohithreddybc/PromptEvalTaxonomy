...
aware of three concurrent works that adopt a measurement-theoretic
lens for adjacent scopes: Bean et al.~\cite{bean2025measuring} apply
construct-validity review to 445 LLM benchmarks broadly; Alaa et
al.~\cite{alaa2025medical} apply it to medical benchmarks
specifically; and Ye et al.~\cite{ye2025psychometrics} survey LLM
psychometrics including validation methodology. Our scope and
contribution are distinct: we apply the validity/reliability tension
specifically to prompt engineering evaluation configurations across
a 152-paper PRISMA corpus and produce a seven-dimensional codebook,
archetype clustering, and failure-mode catalog (FM1--FM3) for that
scope (Section~\ref{subsec:using}). The survey landscape is
fast-moving; the contribution we claim is the proposed organization
and the empirical analyses that follow from it, not field-level
priority on measurement-theoretic LLM evaluation in general.
...
Service Levels) rather than evaluation frameworks; and Dong et
...
prompt structure. Three recent works adopt the same
measurement-theoretic framing we use but target different scopes:
...
across the OpenLLM Leaderboard. These works overlap with our
