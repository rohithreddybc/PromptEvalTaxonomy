...
findings about how those methods co-occur in the literature. To our
knowledge, this is the first systematic survey to organize prompt
engineering evaluation through a measurement-theoretic frame; the
seven-dimensional codebook, failure-mode catalog (FM1--FM3), and
accompanying preregistration checklist are intended as a stable
reference vocabulary that subsequent evaluation work can cite to
disclose methodology and position contributions consistently across
studies (Section~\ref{subsec:using}).
...
explicitly allows for such rounds. The expansion rounds function as
coverage-repair searches that address specific under-represented
topic areas identified after coding the prior set; they are not
independent prevalence sweeps of the full literature, and the
directional bias they introduce is named explicitly as a limitation
in Section~\ref{subsubsec:lim_expansion}.
...
\textit{Failure mode 1 (FM1: Construct Mismatch): task-specific
metric on a non-exact-match construct.} Dimensional fingerprint: D1=Benchmark, D3=Task-specific
...
\textit{Failure mode 2 (FM2: Unpinned Judge): LLM-judge score
without judge-version pinning.} Dimensional fingerprint: D1=LLM-as-Judge, D3=LLM-judge,
...
single-prompt cross-model claim.}
...
standardization is at least describable. Adoption is feasible
immediately: disclosing the D1--D7 configuration of any reported
evaluation takes a single methodology line
(Section~\ref{subsec:using}) and makes the design directly
comparable to prior work without changing the evaluation itself.
The distance from describable to standardized is a coordination
problem, one that requires community adoption of shared standards
