...
be measured or how. We trace this gap to two unresolved tensions from
measurement theory: construct validity vs.\ measurement reliability,
and reference-anchored vs.\ judgment-anchored evaluation. Existing
...
covering evaluation method (D1), task scope (D2), metric type (D3),
automation level (D4), evaluation scope (D5), robustness (D6), and
reproducibility (D7). Applied to a 152-paper PRISMA~2020 corpus from
six academic databases (an initial 79-paper set expanded through four
supplementary rounds covering LLM-judge quality, benchmark
...
calibration, and clinical/legal domain evaluation), the taxonomy
identifies three recurring evaluation archetypes validated by
silhouette analysis and unsupervised $k$-means clustering
(Benchmark-Automation, Judge-Mediated, Expert-Anchored), strong
dimension-pair dependencies (Cram\'{e}r's~$V$ up to 0.702), and
three named failure modes (FM1: Construct Mismatch; FM2: Unpinned
Judge; FM3: Single-Phrasing Generalization) where a paper's
evaluation configuration does not support its stated contribution
claims.

Three patterns emerge from the corpus. The dominant archetype,
Benchmark-Automation (65.7\% of coded papers, 95\% bootstrap CI
[57.9\%, 73.0\%]), occupies the high-reliability, narrow-validity
region of the design space: well-matched to tasks with ground-truth
answers, but under-specifying the construct for open-ended
generation. The Judge-Mediated archetype (17.2\%) addresses this gap
at the cost of introducing judge-model reliability concerns that the
surveyed corpus has not yet managed with a shared calibration
protocol. Robustness and contamination-resistant evaluation remain
minority practices in the corpus, indicating these dimensions are not
yet standard in how prompt engineering evaluation is designed and
reported.

The taxonomy is not a new evaluation method. To our knowledge, it is
the first systematic treatment of prompt engineering evaluation
organized through a measurement-theoretic frame, providing a stable
reference vocabulary (D1--D7, FM1--FM3, and three archetype labels)
that subsequent work can cite to disclose methodology and position
contributions relative to prior designs. A companion seven-question
checklist operationalizing all seven dimensions supports immediate use
in study design and peer review (Table~\ref{tab:quickref}).
\end{abstract}
...
The current taxonomy retains the original five and adds
\textit{D6 (Robustness)} and \textit{D7 (Reproducibility and
Contamination Resistance)}; it also reframes \textit{D4 (Automation
Level)} as a sub-axis of reliability rather than an independent
choice. The seven-dimensional structure is designed to make the two
...
The taxonomy is useful only when researchers can apply it both to
classify existing work and to structure new evaluation designs.
Subsection~\ref{subsec:classify} works through two corpus papers
dimension by dimension; Subsection~\ref{subsec:design} walks forward
through a new study, surfacing design decisions that would otherwise
remain implicit; Subsection~\ref{subsec:quickref} distills the
guidance into one reference table.
...
at least one quality condition in Section~\ref{subsec:inclusion}
(peer review, 10+ Google Scholar citations at screening time, or
recognized NLP/AI venue). Two are explicit 2026 arXiv preprints
(\cite{bellibatlu2026judgesense, benchmark2sq2026}) flagged as
preliminary, with no headline claim resting solely on either. The
remaining 150 mix peer-reviewed conference/journal papers,
foundation-model technical reports, and arXiv preprints meeting the
citation- or venue-criterion threshold. A precise peer-reviewed
vs.\ arXiv split is not reported because many corpus papers were
initially posted to arXiv and subsequently accepted at venues
(NeurIPS, ACL, EMNLP, ICLR, TACL, etc.) while retaining the arXiv
identifier in their BibTeX \texttt{journal} field; an automatic count
would systematically undercount peer-reviewed status. The complete
per-paper record is in \texttt{references.bib} in the artifact
repository.
...
Mapping the corpus against the taxonomy serves three purposes:
testing the taxonomy's expressive coverage of evaluation choices
observed under our inclusion criteria, deriving co-occurrence
patterns / recurring \textit{evaluation archetypes} / a failure-mode
catalog beyond per-dimension counts, and grounding the synthesis in
Section~\ref{sec:discussion}.

\paragraph{PE-core papers vs.\ evaluation infrastructure.} The
corpus contains two overlapping populations: 76 \emph{PE-core
papers} (technique, sensitivity/code prompting, PE-specific surveys)
and 76 \emph{evaluation infrastructure papers} (benchmarks, datasets,
LLM-judge frameworks, metric-quality studies); together they form
the 152-paper corpus. The mixed inclusion is justified in
Section~\ref{subsec:inclusion}: technique papers consume
infrastructure and infrastructure papers shape what techniques can
be evaluated. Aggregate distributions below use the full corpus;
the D7 table (Section~\ref{subsec:d7dist}) additionally reports the
LLM-as-Judge subset where the contrast is most pronounced.

Sections~\ref{subsec:d1dist}--\ref{subsec:d7dist} report per-dimension
distributions directly reproducible from \texttt{corpus\_d1d7.csv},
with subset contrasts noted inline. Section~\ref{subsec:cooccurrence}
reports the co-occurrence matrix and the configurations accounting
for the bulk of the corpus. Section~\ref{subsec:archetypes} names
the recurring archetypes and locates each on the validity-reliability
frame (Section~\ref{subsec:tensions}). Section~\ref{subsec:failuremodes}
walks through three worked failure modes where the taxonomy makes a
methodological mismatch visible.

Table~\ref{tab:archetype_distribution} previews the five most common
configurations and the tension each foregrounds. A representative
ten-paper mapping is in the Electronic Supplement
(Appendix~B, Table~B.1); the full 152-paper mapping is in Appendix~C;
the machine-readable dataset is included as a review-time snapshot
with the submission and will be tagged citably at acceptance.
Table~\ref{tab:comparison} summarizes the modal configuration per
paper category.
...
documents large paraphrase-induced variance but has not propagated
into the broader corpus as a routine reporting requirement.
